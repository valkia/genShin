<template>
    <div class="main-app">
        ...
        <el-dialog title="" v-model="showResultFlag">
            <ResultPage :calculating="calculating"
                        :result-data="resultData"></ResultPage>
        </el-dialog>
...
<script setup lang="ts">
    ...
    const exchangeArtifacts = (type: PositionChnName, item: any) => {
        ...
        console.log(tagChange(tag0Key, tag0Value))
        console.log(valueChange(tag0Value))
    ...
    let showResultFlag = ref(false)
    let calculating = ref(true)
    let resultData = ref({})
